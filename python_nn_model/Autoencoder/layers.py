    ...
    def conv_encoder(k_screens):
        ...
        with tf.variable_scope("encoder"):
            with tf.variable_scope("cnn_layers", reuse=tf.AUTO_REUSE):
                ...
                w1 = tf.get_variable("conv1_weights", [8, 8, 5, 32], initializer=tf.random_uniform_initializer(-0.1,0.1))
                conv1 = tf.nn.relu(tf.nn.conv2d(NHWC_k_screens, w1, strides=[1, 2, 2, 1], padding='SAME'))
                cnn_shapes.append(conv1.get_shape().as_list())

                w2 = tf.get_variable("conv2_weights", [6, 6, 32, 64], initializer=tf.random_uniform_initializer(-0.1,0.1))
                conv2 = tf.nn.relu(tf.nn.conv2d(conv1, w2, strides=[1, 2, 2, 1], padding='SAME'))
                cnn_shapes.append(conv2.get_shape().as_list())

                w3 = tf.get_variable("conv3_weights", [6, 6, 64, 64], initializer=tf.random_uniform_initializer(-0.1,0.1))
                conv3 = tf.nn.relu(tf.nn.conv2d(conv2, w3, strides=[1, 2, 2, 1], padding='SAME'))
                cnn_shapes.append(conv3.get_shape().as_list())

                w4 = tf.get_variable("conv4_weights", [4, 4, 64, 64], initializer=tf.random_uniform_initializer(-0.1,0.1))
            ...
            with tf.variable_scope("flatten_layers", reuse=tf.AUTO_REUSE):
                ...
                w = tf.get_variable("dense_weights", [14*10*64, 2048], initializer=tf.random_uniform_initializer(-0.1,0.1))
    ...
    def conv_decoder(pred_encode, cnn_shapes):
        with tf.variable_scope("decoder"):
            with tf.variable_scope("dense_layers", reuse=tf.AUTO_REUSE):
                w = tf.get_variable("dense_weights", [2048, 14*10*64], initializer=tf.random_uniform_initializer(-0.1,0.1))
                b = tf.get_variable("dense_bias", [14*10*64], initializer=tf.random_uniform_initializer(-0.1,0.1))
            ...
            with tf.variable_scope("cnn_transpose_layers", reuse=tf.AUTO_REUSE):
                w1 = tf.get_variable("conv_transpose1_weights", [4, 4, 64, 64], initializer=tf.random_uniform_initializer(-0.1,0.1))
                ...
                w2 = tf.get_variable("conv_transpose2_weights", [6, 6, 64, 64], initializer=tf.random_uniform_initializer(-0.1,0.1))
                ...
                w3 = tf.get_variable("conv_transpose3_weights", [6, 6, 32, 64], initializer=tf.random_uniform_initializer(-0.1,0.1))
