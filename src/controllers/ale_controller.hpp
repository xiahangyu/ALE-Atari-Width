...
class ALEController {
  ...
  protected:
    ...
    /** Added by xhy, for background histogram*/
    void count_bghist();
    /** Added by xhy, get background matrix from histogram*/
    void count_bgMatrix();

    /** xhy, save background as png*/
    void save_bg(const string& filename);
    /** xhy, save background as matrix*/
    void saveBgAsMatrix(const string& filename);
    /** xhy, load background matrix from matrix file*/
    void loadBgFromMatrix(const string& filename);

    /** Added by xhy, for saving screens. */
    void save_screen(const string& filename);
    /** Added by xhy*/
    void saveScreenAsMatrix(const string& filename, const string& matrix_act_fn, Action& action_a);
